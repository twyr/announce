...
class PublishCommandClass {
		...
		const taskList = new Listr(
			[{
				...
				},
				{
					...
					'skip': (ctxt) => {
						if(!ctxt?.options?.npmToken?.trim?.()?.length)
							return `Cannot publish without an NPM token.`;
						if(!ctxt?.options?.releaseToBePublished)
							return `Cannot publish without a release on the Git host.`;
						if(!ctxt?.options?.releaseToBePublished?.tarball_url)
							return `Cannot publish a release without a tarball.`;
	...
	async _getUpstreamRepositoryInfo(ctxt, task) {
		...
		const releaseToBePublished =
			await gitHostWrapper?.fetchReleaseInformation?.(
				repository,
				ctxt?.options?.releaseName
			);
		if(!releaseToBePublished)
			throw new Error(`Unknown Release: ${ctxt?.options.releaseName}`);
	...
	async _publishToNpm(ctxt, task) {
		let distTag = null;
		if(
			(ctxt?.options?.distTag ?? 'version_default') === 'version_default'
		) {
		...
		publishOptions?.push?.(
			ctxt?.options?.releaseToBePublished?.tarball_url
		);
...
exports.commandCreator = function commandCreator(
	...
	const projectPackageJson = path.join(
		configuration?.publish?.currentWorkingDirectory?.trim?.() ??
		process.cwd(),
		'package.json'
	);
	...
	if(pkg) {
		...
		if(configuration?.publish?.currentWorkingDirectory) {
			configuration.publish.currentWorkingDirectory = fillTemplate?.(
				configuration?.publish?.currentWorkingDirectory,
				pkg
			);
		}

		if(configuration?.publish?.releaseName) {
			configuration.publish.releaseName = fillTemplate?.(
				configuration?.publish?.releaseName,
				pkg
			);
		}
	}
	...
	publish
		...
		.option(
			'--access <level>',
			'Public / Restricted',
			configuration?.publish?.access?.trim?.() ?? 'public'
		)
		.option(
			'--dist-tag <tag>',
			'Tag to use for the published release',
			configuration?.publish?.distTag?.trim?.() ?? 'latest'
		)
		.option(
			'--dry-run',
			'Dry run publish',
			configuration?.publish?.dryRun ?? false
		)

		.option(
			'--github-token <token>',
			'Token to use for accessing the release on GitHub',
			configuration?.publish?.githubToken?.trim?.() ??
			process.env.GITHUB_TOKEN ??
			'PROCESS.ENV.GITHUB_TOKEN'
		)
		.option(
			'--gitlab-token <token>',
			'Token to use for accessing the release on GitLab',
			configuration?.publish?.gitlabToken?.trim?.() ??
			process.env.GITLAB_TOKEN ??
			'PROCESS.ENV.GITLAB_TOKEN'
		)
		.option(
			'--npm-token <token>',
			'Automation Token to use for publishing the release to NPM',
			configuration?.publish?.npmToken?.trim?.() ??
		...
		.option(
			'--release-name <name>',
			'Release name on the Git Host for fetching the compressed assets',
			configuration?.publish?.releaseName?.trim?.() ??
			(pkg ? `V${pkg?.version} Release` : 'Release')
		)
		.option(
			'--upstream <remote>',
			'Git remote to use for accessing the release',
			configuration?.publish?.upstream ?? 'upstream'
		);
